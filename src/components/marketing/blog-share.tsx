...
export function BlogShare({ title, slug }: { title: string; slug: string }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-sm text-brand-midnight/50 mr-2">Share</span>
      {shareLinks.map((s) => (
        <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={`Share on ${s.label}`}>
          <Button variant="outline" size="sm" className="rounded-full border-brand-midnight/10 h-9 px-3 gap-1.5">
            {s.icon ? <s.icon className="w-4 h-4" /> : <span className="text-xs font-bold">WA</span>}
            <span className="text-xs hidden sm:inline">{s.label}</span>
          </Button>
        </a>
      ))}
      <Button variant="outline" size="sm" className="rounded-full border-brand-midnight/10 h-9 px-3 gap-1.5" onClick={copyLink}>
